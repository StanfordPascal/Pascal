...
(*    or binary, and others normal char representation              *)
(*                                                                  *)
...
(*    should be allowed as argument to strings, not only            *)
...
var TRACEF : TEXT ;
    ...
    (* CHNEXT - next character                             *)
    (* SY     - symbol read                                *)
    (* OP     - operator symbol                            *)
    (* VAL    - constant (if symbol was constant)          *)
    (* LENGTH - length of symbol or constant               *)
    (*******************************************************)
...
(**************************************************************)
   ...
   (******************************************************)
      ...
      begin (* SKIPBLNK *)
        ...
        (***********************************************)
          ...
          EOL := CHCNT > LASTCOL ;
        until not EOL ;
        SCANCH := LINEBUF [ CHCNT ] ;
        SCANCHNEXT := LINEBUF [ CHCNT + 1 ] ;
      end (* SKIPBLNK *) ;
   ...
   (***********************************)
      ...
      begin (* OPTIONS *)
        ...
        repeat
          NEXTCH ;
          if SCANCH in LOW_LETTERS + UP_LETTERS then
            begin
              SCH := UPSHIFT [ SCANCH ] ;
              NEXTCH ;
              case SCH of
   ...
   procedure INSTRING ( var STRTYPE : CHAR ) ;

      var XSTRING : array [ 1 .. MAXSTRL ] of CHAR ;
          FERTIG : BOOLEAN ;
          XSTRINGPART : array [ 1 .. MAXSTRL ] of CHAR ;
          LPART : INTEGER ;
          STRTYPEPART : CHAR ;
                     ...
                     for X := 1 to LNEU do
                       begin
                         ...
                         for X2 := 1 to L1 do
                           begin
                             INEU := INEU * 2 ;
                             if XSTRINGPART [ IX ] = '1' then
                               INEU := INEU + 1 ;
                             IX := IX + 1 ;
                           end (* for *) ;
                         L1 := 8 ;
                         XSTRINGPART [ X ] := CHR ( INEU ) ;
                       end (* for *) ;
                     L := LNEU ;
                   end (* tag/ca *) ;
             'X' : begin
                     ...
                     for X := 1 to LNEU do
                       begin
                         ...
                         for X2 := 1 to L1 do
                           begin
                             INEU := INEU * 16 ;
                             if XSTRINGPART [ IX ] in [ '1' .. '9' ]
                             then
                               INEU := INEU + ORD ( XSTRINGPART [ IX ]
                                       ) - ORD ( '0' )
                             else
                               if XSTRINGPART [ IX ] in [ 'A' .. 'F' ]
                               then
                                 INEU := INEU + ORD ( XSTRINGPART [ IX
                                         ] ) - ORD ( 'A' ) + 10 ;
                             IX := IX + 1 ;
      ...
      begin (* INSTRING *)
        ...
        (*****************************************)
        (*   wenn zeilenende, dann fehler 202    *)
        (*   und zeilenende erzwingen durch      *)
        (*   einstellen von hochkomma            *)
        (*****************************************)
            ...
            case STRTYPEPART of
              'B' : begin
                      if SCANCH = '_' then
                        continue ;
                      if SCANCH in BIN_CHARS then
                        begin
                          LPART := LPART + 1 ;
                          if LPART <= MAXSTRL then
                            XSTRINGPART [ LPART ] := SCANCH
                        end (* then *)
                      else
                        ERROR ( 210 )
                    end (* tag/ca *) ;
              'X' : begin
                      if SCANCH = '_' then
                        continue ;
                      if SCANCH in HEX_CHARS then
                        begin
                          LPART := LPART + 1 ;
                          if LPART <= MAXSTRL then
                            XSTRINGPART [ LPART ] := UPSHIFT [ SCANCH ]
                        end (* then *)
                      else
                        ERROR ( 209 )
                    end (* tag/ca *) ;
          ...
          if LPART <= MAXSTRL then
            begin
              case STRTYPEPART of
                'B' : begin
                        MODSTRING ( STRTYPEPART , LPART ) ;
                      end (* tag/ca *) ;
                'X' : begin
                        MODSTRING ( STRTYPEPART , LPART ) ;
                      end (* tag/ca *)
              end (* case *) ;
              if LENGTH + LPART <= MAXSTRL then
                begin
                  MEMCPY ( ADDR ( XSTRING [ LENGTH + 1 ] ) , ADDR (
                           XSTRINGPART ) , LPART ) ;
                  LENGTH := LENGTH + LPART
                end (* then *)
              else
          ...
          case SCANCH of
            ...
            'X' , 'B' , 'x' , 'b' :
              begin
                ...
                if not FERTIG then
                  begin
                    STRTYPEPART := UPSHIFT [ SCANCH ] ;
                    if STRTYPE <> STRTYPEPART then
                      STRTYPE := 'X' ;
                    NEXTCH ;
                  end (* then *)
              end (* tag/ca *) ;
        ...
        (******************************************)
        (*   NOW LENGTH = NR OF CHARS IN STRING   *)
        (******************************************)
        (*   modify LENGTH in case of hex or bin  *)
        (******************************************)
   ...
   begin (* INSYMBOL *)
     ...
     (**********************************)
           ...
           ID [ 1 ] := CHSAVE ;
           K := 1 ;
           while SOP [ SCANCH ] in [ ATOZCH , NUMCH , UNDSCH , DOLLARCH
           ] do
             begin
               if K < IDLNGTH then
                 begin
     ...
     (**************************************)
                   ...
                   if STATE = 8 then
                     if ( SOP [ SCANCH ] = NUMCH ) or ( SCANCH in [ 'A'
                     .. 'F' ] ) or ( SCANCH in [ 'a' .. 'f' ] ) then
                       goto 3 ;
