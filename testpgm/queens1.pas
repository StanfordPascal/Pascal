...
procedure WRITE_SET ( S : set of - 10 .. 10 ; SHOWDUMP : BOOLEAN ) ;
...
procedure FINDQUEENS ( NEWQUEENROW : CRANGE ;                      //
   ...
   begin (* FINDQUEENS *)
     ...
     //****************************
         ...
         if FALSE then
           begin
             WRITE ( AUFNRLOK : 6 , ' check: ' , NEWQUEENROW : 2 ,
                     TRIALCOL : 2 ) ;
             WRITE ( ' cols: ' ) ;
             WRITE_SET ( COLS , TRUE ) ;
             WRITE ( ' updiag: ' ) ;
             WRITE_SET ( UPDIAG , TRUE ) ;
             WRITE ( ' downdiag: ' ) ;
             WRITE_SET ( DOWNDIAG , TRUE ) ;
             WRITELN
           end (* then *) ;
           ...
           begin
             ...
             if NEWQUEENROW < CMAX then
               begin
                 if FALSE then
                   begin
                     WRITE ( AUFNRLOK : 6 , ' rekursiv weiter' ) ;
                     WRITE ( ' cols: ' ) ;
                     WRITE_SET ( COLS , TRUE ) ;
                     WRITE ( ' updiag: ' ) ;
                     WRITE_SET ( UPDIAG , TRUE ) ;
                 ...
                 if FALSE then
                   begin
                     WRITE ( AUFNRLOK : 6 , ' rekursiv zurueck' ) ;
                     WRITE ( ' cols: ' ) ;
                     WRITE_SET ( COLS , TRUE ) ;
                     WRITE ( ' updiag: ' ) ;
                     WRITE_SET ( UPDIAG , TRUE ) ;
